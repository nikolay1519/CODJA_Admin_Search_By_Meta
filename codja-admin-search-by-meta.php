<?php
	...
	class Codja_Admin_Search_By_Meta {
		...
		private function getAcfFields( $post_types ) {
			$acf_version = intval( acf()->version );
			$fields = array();

			if ( $acf_version == 4 ) {
				// Get fields of ACF free
				require_once( CJ_ASM_DIR . '/includes/acf_free.class.php' );
				$fields = Codja_ASM_Acf_Free::getInstance()->getFields( $post_types );
			} elseif ( $acf_version > 4 ) {
				// Get fields of ACF pro
				require_once( CJ_ASM_DIR . '/includes/acf_pro.class.php' );
				$fields = Codja_ASM_Acf_Pro::getInstance()->getFields( $post_types );
			}
